...
func (c *Coordinator) SearchObservations(ctx context.Context, filter ObservationFilter) ([]domain.Observation, error) {
	scopedContext, scopedCampaign := c.queryScopes.Begin(ctx, filter.CampaignID)
	defer c.queryScopes.Release(scopedContext, scopedCampaign)
	if err := scopedContext.Err(); err != nil {
		return nil, err
	}
	items := c.repo.ListObservations(scopedCampaign)
...
func (c *Coordinator) ObservationLabels(ctx context.Context, campaignID string) ([]string, error) {
	scopedContext, scopedCampaign := c.queryScopes.Begin(ctx, campaignID)
	defer c.queryScopes.Release(scopedContext, scopedCampaign)
	if err := scopedContext.Err(); err != nil {
		return nil, err
	}
	items, err := c.ListObservations(scopedContext, scopedCampaign)
	if err != nil {
		return nil, err
	}
