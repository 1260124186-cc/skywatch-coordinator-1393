...
func (c *Coordinator) Progress(ctx context.Context, campaignID string) (CampaignProgress, error) {
	...
	progress := CampaignProgress{CampaignID: campaignID}
	byStation := map[string]*StationProgress{}
	for _, station := range c.repo.ListStations(campaignID) {
	...
	for _, observation := range c.repo.ListObservations(campaignID) {
